...
class Bt:
    ...
    def _strategy_replay(self, cycle_interval=1, **kwargs):
        ...
        pause_status_dir = f"{BASE_DIR}/liveplot/replay/pause_status"
        ...
        async def main(strategies: list[Strategy]):
            max_cycles = max(
                [s._btklinedataset.max_length for s in strategies]) if strategies else -1
    ...
    def run(self, isplot=True, isreport: bool = False, **kwargs) -> Bt:
        ...
        if replay:  # 策略回放
            self._strategy_replay(kwargs.pop("period_milliseconds", 1000))
            return self
        else:
