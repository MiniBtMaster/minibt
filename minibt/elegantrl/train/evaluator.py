...
class Evaluator:
    def __init__(self, cwd: str, env, args: Config, if_tensorboard: bool = False):
        ...
        # args.gpu_id可能是numpy数组（如np.array(0)），需转为int
        self.agent_id = int(args.gpu_id) if isinstance(
            args.gpu_id, (np.ndarray, np.integer)) else args.gpu_id
        ...
        # evaluate the agent per training steps
        self.eval_per_step = args.eval_per_step
        ...
        self.recorder_step = args.eval_record_step
        self.min_start_mode = args.min_start_mode
        self.max_r = -np.inf
        # print("| Evaluator:"
        ...
        # -------------------------- 关键修改1：规整表头格式 --------------------------
        # 每个列设置固定宽度，确保与数据行对齐（数字列右对齐，文字列左对齐）
        print("| 评估器说明:"
              "\n| `step`: 样本数量，或总训练步数，或`env.step()`的运行次数。"
              "\n| `time`: 从训练开始到此刻所花费的时间（秒）。"
              "\n| `avgR`: 累积奖励的平均值，即一个回合中奖励的总和。"
              "\n| `stdR`: 累积奖励的标准差，反映回合奖励的稳定性。"
              "\n| `avgS`: 一个回合中的平均步数（若为固定值，说明未达到终止条件）。"
              "\n| `stdS`: 回合步数的标准差，反映回合长度的稳定性。"
              "\n| `expR`: 期望回报，智能体预测的理论收益。"
              "\n| `objC`: 评论者网络(Critic)的损失值，越小表示价值预测越准确。"
              "\n| `objA`: 行动者网络(Actor)的目标值，越大表示策略越优（需结合任务）。"
              "\n| `其他`: 额外日志信息（如动作分布、状态统计等）。"
              f"\n{'=' * 100}\n"  # 用等号分隔，比#更清晰
              # 表头列宽：ID(3) 步数(12) 时间(8) | 平均回报(10) 回报标准差(8) 平均步数(8) 步数标准差(8) | 期望回报(10) 评论者目标(8) 行动者目标(8) 其他(20)
              f"{'ID':<3}{'步数':>8}{'时间(秒)':>8} |{'平均回报':>8}{'回报标准差':>8}{'平均步数':>8}{'步数标准差':>8} |{'期望回报':>8}{'评论者目标':>8}{'行动者目标':>8}{'其他':>8}",
              flush=True)
    ...
    def evaluate_and_save(self, actor: th.nn, steps: int, exp_r: float, logging_tuple: tuple):
        ...
        self.recorder.append(
            (self.total_step, avg_r, std_r, exp_r, *value_tuple))  # update recorder
        if self.tensorboard:
        ...
        # update max average cumulative rewards
        self.max_r = max(self.max_r, avg_r)
        ...
        print(f"{self.agent_id:<3}{self.total_step:12.2e}{train_time:^9d} |"
              f"{avg_r:12.2f}{std_r:^14.1f}{avg_s:^12.0f}{std_s:^12.0f} |"
              f"{exp_r:12.2f}{''.join(f'{n:12.2f}' for n in value_tuple)} {logging_str}", flush=True)
    ...
    def get_cumulative_rewards_and_step_single_env(self, actor) -> TEN:
        rewards_steps_list = [get_rewards_and_steps(
            self, actor) for _ in range(self.eval_times)]
    ...
    def save_training_curve_jpg(self):
        ...
        draw_learning_curve(recorder=recorder, fig_title=fig_title,
                            save_path=f"{self.cwd}/LearningCurve.jpg")
        # save self.recorder for `draw_learning_curve()`
        np.save(self.recorder_path, recorder)


"""util"""


def _resolve_min_start_length(evaluator: Evaluator, min_start_length: int, max_step: int,
                               min_start_mode: str) -> int:
    """根据 min_start_mode 计算本次评估的最小开始长度。

    Args:
        env: 环境对象（用于存储增量模式的计数器）
        min_start_length: 基础最小开始长度（来自 env 配置）
        max_step: 环境最大步数
        min_start_mode: 开始长度模式

    Returns:
        本次评估使用的最小开始长度
    """
    mode = min_start_mode.lower()
    if mode == "fixed":
        return min_start_length
    elif "random" in mode:
        if "random" == mode:
            return np.random.randint(min_start_length, 2*min_start_length + 1)
        increment_str = mode.replace("random", "")
        # 提取字符串中的数字部分
        increment = int(''.join(filter(str.isdigit, increment_str))) if increment_str else 2*min_start_length
        if increment <= min_start_length:
            increment = 2*min_start_length
        return min(np.random.randint(min_start_length, increment + 1), max_step // 2)

    elif "incremental" in mode:
        if "incremental" == mode:
            increment = 1
        else:
            # 从模式中提取增量步长（如 "incremental10" -> 增量为10）
            increment_str = mode.replace("incremental", "")
            # 提取字符串中的数字部分
            increment = int(''.join(filter(str.isdigit, increment_str)))
        counter = evaluator.min_start_length
        evaluator.min_start_length += increment
        return min(counter, max_step // 2)

    return min_start_length


def get_rewards_and_steps(evaluator: Evaluator, actor, if_render: bool = False) -> Tuple[float, int]:
    """Usage
    eval_times = 4
    net_dim = 2 ** 7
    ...
    """
    env = evaluator.env
    min_start_mode = evaluator.min_start_mode
    ...
    episode_steps = 0
    min_start_length = 0
    if hasattr(env, "min_start_length"):
        min_start_length = env.min_start_length

    min_start_length = _resolve_min_start_length(
        env, min_start_length, max_step, min_start_mode)

    cumulative_returns = 0.0
    for episode_steps in range(min_start_length, max_step):
        tensor_state = th.as_tensor(
            state, dtype=th.float32, device=device).unsqueeze(0)
    ...
    cumulative_returns = getattr(
        env_unwrapped, 'cumulative_returns', cumulative_returns)
...
def get_cumulative_rewards_and_step_from_vec_env(env, actor) -> List[Tuple[float, int]]:
    ...
    if hasattr(env, 'cumulative_returns'):  # GPU
        returns_step_list = [(ret, env.max_step)
                             for ret in env.cumulative_returns]
    else:  # CPU
        returns = returns.cpu()
        dones = dones.cpu()
        ...
        for i in range(env_num):
            ...
            for j1 in dones_where.tolist():
                # cumulative returns of an episode
                reward_sum = returns[j0:j1, i].sum().item()
...
def demo_evaluator_actor_pth():
    ...
    args = Config(agent_class=agent_class,
                  env_class=env_class, env_args=env_args)
    env = build_env(env_class=args.env_class, env_args=args.env_args)
    act = agent_class(net_dim, env.state_dim, env.action_dim,
                      gpu_id=gpu_id, args=args).act
...
def demo_evaluate_actors(dir_path: str, gpu_id: int, agent, env_args: dict, eval_times=2, net_dim=128):
    ...
    for act_name in act_names:
        ...
        act.load_state_dict(
            th.load(act_path, map_location=lambda storage, loc: storage))
        r_s_ary = [get_rewards_and_steps(env, act) for _ in range(eval_times)]
        r_s_ary = np.array(r_s_ary, dtype=np.float32)
        # average of episode return and episode step
        r_avg, s_avg = r_s_ary.mean(axis=0)
...
def demo_load_pendulum_and_render():
    ...
    act = agent_class(net_dim, env.state_dim, env.action_dim,
                      gpu_id=gpu_id, args=args).act
    act.load_state_dict(torch.load(
    ...
    for steps in range(max_step):
        ...
        a_tensor = act(s_tensor).argmax(
            dim=1) if if_discrete else act(s_tensor)
        # not need detach(), because using th.no_grad() outside
...
def run():
    ...
    if if_show_episode_step:
        ...
        ax_twin.fill_between(plot_x, 0, plot_y_step,
                             facecolor=color1, alpha=0.3)
