...
class Bt:
    ...
    def _strategy_replay(self, **kwargs):
        ...
        for i, s in enumerate(self.strategies):
            ...
            s._get_plot_datas()
            s._first_start = True
            # ===== DEBUG: 记录 init_datas 结构到文件 =====
            # _pd = s._plot_datas
            # with open(r'd:\newminibt\debug_init_datas.txt', 'a', encoding='utf-8') as _f:
            #     _f.write(f"\n策略: {_pd[0]}\n")
            #     _f.write(f"K线数据数量: {len(_pd[1])}\n")
            #     for ji, df in enumerate(_pd[1]):
            #         _f.write(f"  K线[{ji}] 类型={type(df).__name__} 长度={len(df) if hasattr(df, '__len__') else 'N/A'}\n")
            #         if hasattr(df, 'index'):
            #             _f.write(f"  K线[{ji}] 索引范围: {df.index.min()} ~ {df.index.max()}\n")
            #             _f.write(f"  K线[{ji}] 列: {list(df.columns)}\n")
            #     _f.write(f"指标数据数量: {len(_pd[2])}\n")
            #     for ji, inds in enumerate(_pd[2]):
            #         _f.write(f"  合约[{ji}] 有 {len(inds)} 个指标\n")
            #         for ik, ind in enumerate(inds):
            #             indicator_arr = ind[7]
            #             _f.write(f"    指标[{ik}] {ind[4]} 类型={type(indicator_arr).__name__} shape={getattr(indicator_arr, 'shape', 'N/A')}\n")
            # ===== END DEBUG =====
    ...
    def optstrategy(self, target: OPTTargetType = 'profit_ratio',
                    weights: float | tuple[float] = 1., opconfig: OpConfig | OptunaConfig | dict | list[dict] = {}, op_method: OPTMethodType = 'optuna', show_bar=True, skip=False, **kwargs):
    ...
    def run(self, isplot=True, isreport: bool = False, **kwargs) -> Bt:
        ...
        [(t._print_account, t.richprint) for t in self.strategies
