...
class PairTrading(LazyImport):
    ...
    @staticmethod
    def percentage_deviation_strategy(spread_series: pd.Series, window=60, threshold=0.1, **kwargs) -> pd.Series | pd.DataFrame:
        """使用百分比偏差生成交易信号"""
        spread_mean = spread_series.rolling(window=window).mean()
    ...
    @staticmethod
    def z_score_strategy(spread_series: pd.Series, window=60, z_threshold=2.0, **kwargs) -> pd.DataFrame | pd.Series:
        """使用Z-score生成交易信号"""
        spread_mean = spread_series.rolling(window=window).mean()
        spread_std = spread_series.rolling(window=window).std()
    ...
    @staticmethod
    def hurst_filter_strategy(spread_series,window=20, hurst_threshold=0.5, z_threshold=2.0, **kwargs) -> pd.DataFrame:
        """使用Hurst指数过滤交易信号"""
    ...
    @classmethod
    def kalman_filter(cls, x_series: pd.Series, y_series: pd.Series, window=60, 
                      state_var_init=1e-4, obs_var=1e-2, **kwargs) -> pd.DataFrame:
        """使用卡尔曼滤波生成动态价差和交易信号"""
        price_x = x_series.values      # leg[0] 价格（v）
        price_y = y_series.values     # leg[1] 价格（pp）
        size = len(price_x)
    ...
    @staticmethod
    def vecm_based_signals(x_series, y_series, window=60, lag=2, **kwargs) -> pd.DataFrame | pd.Series:
        """完全手动实现的VECM模型，确保输入数据长度一致"""
        # 数据预处理：转换为数值类型并删除缺失值
        # 兼容 numpy 数组和 pandas Series
        if not isinstance(x_series, pd.Series):
            x_series = pd.Series(np.asarray(x_series).flatten())
        if not isinstance(y_series, pd.Series):
            y_series = pd.Series(np.asarray(y_series).flatten())
