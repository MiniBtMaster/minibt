...
class Strategy(Base, StrategyBase, metaclass=StrategyMeta):
    ...
    def __call__(self: Strategy, *args, **kwds) -> Any:
        """
        策略实例调用入口（模式分发器）
                ...
                if signal_info:
                    for k, v in signal_info.items():
                        ...
                        if signalshow:
                            ...
                            sindex = ind.index[svalues > 0].values
                            sprice = price[svalues > 0]
                            sdatetime = _datetime[svalues > 0]
                            slen = len(sindex)
                            signal_data = dict(
                                index=sindex,
                                datetime=sdatetime,
