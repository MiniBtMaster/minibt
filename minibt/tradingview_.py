...
class Khaled_Tamim_Avellaneda_Stoikov_Strategy(BtIndicator):
    ...
    def next(self):
        def avellanedaStoikov(src: series, gamma=0., sigma=0., T=0., K=0., **Kwargs):
            midPrice = src.sma(3)
...
class Aroon_and_ASH_strategy(BtIndicator):
    ...
    def next(self):
        upper_long: series = 100. * (self.high.rolling(self.params.length_upper_long+1).apply(lambda x: int(
            np.argmax(x[::-1]))) + self.params.length_upper_long) / self.params.length_upper_long
        lower_long: series = 100. * (self.low.rolling(self.params.length_lower_long + 1).apply(lambda x: int(
            np.argmin(x[::-1]))) + self.params.length_lower_long) / self.params.length_lower_long

        upper_short: series = 100. * (self.high.rolling(self.params.length_upper_short + 1).apply(lambda x: int(
            np.argmax(x[::-1]))) + self.params.length_upper_short) / self.params.length_upper_short
        lower_short: series = 100. * (self.low.rolling(self.params.length_lower_short + 1).apply(lambda x: int(
...
class Crunchster_Normalised_Trend_Strategy(BtIndicator):
    ...
    def next(self):
        diff: series = self.close-self.close.shift()
        ...
        for i, value in enumerate(nret):
            if not np.isnan(value):
                cums += value
                nprice[i] = cums
        # nprice = self.to_series(nprice, self.close.ind_setting(name="nprice"))
        nprice: series = series(nprice)
...
class RSI_versus_SMA(BtIndicator):
    ...
    @staticmethod
    def switchDelay(exp: series, len):
        avg = exp.sma(len)
        stats = exp > avg
...
class Open_Close_Cross_Strategy(BtIndicator):
    ...
    def variant(self, src: series, c: np.ndarray):
        size = src.size
        v = np.zeros(size)
        src = src.values
        for i in range(size):
...
class Multi_Deviation_Scaled_Moving_Average(BtIndicator):
    ...
    def dsma(self, src: series, fac: np.ndarray, period: int):
...
class MadTrend(BtIndicator):
    ...
    def calc_src(self, src: str) -> series:
        try:
    ...
    def mad(self, length: int) -> series:
        med = self.close.rolling(window=length).median()  # // Calculate median
        # // Calculate absolute deviations from median
        abs_deviations: series = (self.close - med).abs()
    ...
    def next(self):
        ...
        direction: series = series(np.zeros(src.size))
...
class Dual_Bayesian_For_Loop(BtIndicator):
    ...
    def next(self):
        ...
        loop_score: series = source.rolling(
...
class RSI_Trail(BtIndicator):
    ...
    def calculate_bounds(self, ma, range, upper, lower) -> tuple[series]:
...
class Adaptive_Trend_Flow_Strategy_with_Filters(BtIndicator):
    ...
    def calculate_trend_levels(self) -> tuple[series]:
        typical = self.hlc3()
    ...
    def get_trend_state(self, upper: series, lower: series, basis: series) -> tuple[series]:
        size = self.close.size
        ...
        return series(trend), series(prev_level)
...
class RSI_over_screener_6_TABLO(BtIndicator):
    ...
    def next(self):
        ...
        dif = dataframe(self[["low", "high"]].values, lines=["low", "high"])
...
class Martingale_Short(BtIndicator):
    ...
    def next(self):
        b1: series = self.close.shift(1) * (1 + self.params.buffer / 100.)
        b2: series = self.close.shift(1) * (1 - self.params.buffer / 100.)
...
class SuperATR_7Step_Profit_Strategy(BtIndicator):
    ...
    def next(self):
        ...
        momentum_factor: series = normalized_momentum.abs()
        # self.close.mom()
        ...
        trend_signal: series = ((short_ma > long_ma) & (trend_strength > self.params.trend_strength_threshold)).apply(lambda x: x and 1. or 0.) +\
...
class Dynamic_RSI_Mean_Reversion_Strategy(BtIndicator):
    ...
    def next(self):
        ...
        dynamicOB: series = self.params.baseOBLevel + \
            (normalizedATR * (100. - self.params.baseOBLevel) * self.params.atrScaling)
        dynamicOS: series = self.params.baseOSLevel - \
...
class Reflected_ema_Difference(BtIndicator):
    ...
    def next(self):
        ...
        valor_reflejado_delta: series = ema_corta_delta + \
...
class Kaufman_Adaptive_Moving_Average_KAMA_Strategy(BtIndicator):
    ...
    def rising(self, source: series, period):
    ...
    def falling(self, source: series, period):
        result = source < source.shift()
        if period <= 1:
            return result
        for i in range(1, period+1):
...
class Triple_EMA_QQE_Trend_Following_Strategy(BtIndicator):
    ...
    def next(self):
        ...
        atrRsi: series = (rsiMa.shift() - rsiMa).fillna(method="bfill").abs()
...
class RSI_and_ATR_Trend_Reversal_SL_TP(BtIndicator):
    ...
    def next(self):
        ...
        rsilower: series = self.close.rsi(self.params.rsi_length)
...
class GT_5_1_Strategy(BtIndicator):
    ...
    def f_zrsi(self, _source: series, _length) -> series:
        return _source.rsi(_length) - 50.

    def f_zstoch(self, _source: series, _length, _smooth, _scale) -> series:
    ...
    def f_rsi(self, _source: series, _length, _mode) -> series:
        size = _source.size
    ...
    def f_rsiHeikinAshi(self, _length) -> tuple[series]:
        ...
        data = dataframe(np.c_[(_openRSI, _highRSI, _lowRSI, _close)], lines=(
            "open", "high", "low", "close"))
        return data.ha().to_lines()

    def Coral_Trend_Candles(self) -> series:
    ...
    def next(self):
        ...
        sslUp, sslDown = series(sslUp), series(sslDown)
...
class Faytterro_Estimator_Strategy(BtIndicator):
    ...
    def next(self):
        cr: series = self.close.rolling(2*self.params.len-1).apply(self.cr)
...
class SSL_Wave_Trend_Strategy(BtIndicator):
    ...
    # //EDSMA
    @staticmethod
    def get2PoleSSF(src: series, length) -> np.ndarray:
        src = src.values
        PI = 2 * math.asin(1)
    ...
    @staticmethod
    def get3PoleSSF(src: series, length) -> np.ndarray:
        src = src.values
        PI = 2 * math.asin(1)
    ...
    def getma(self, type: str, src: series, len):
        size = src.size
    ...
    def function(self, source: series, length):
    ...
    @staticmethod
    def formula(number: series, decimals) -> float:
...
class Bitcoin_Scalping_Strategy(BtIndicator):
    ...
    def f_ma_val(self, _price: series, _typ: str, _len: int) -> series:
    ...
    def f_pmarp(self, _price: series, _pmarLen, _pmarpLen, _type) -> series:
        _pmar: series = _price.ZeroDivision(
...
class RedK_Slow_Smooth_Average(BtIndicator):
    ...
    def f_LazyLine(self, _data: series, _length: int):
        w1 = 0
        w2 = 0
        w3 = 0
        L1 = 0.0
...
class Dynamic_Swing_Anchored_VWAP(BtIndicator):
    ...
    def calculate_dynamic_swing_vwap(self, df, prd=50, baseAPT=20, useAdapt=False, volBias=10.0, atrLen=50):
        ...
        返回：
        df:        新增列 ['ph','pl','phL','plL','dir','apt_series','vwap_green','vwap_red']
        """
        ...
        df['apt_series'] = np.round(apt_clamped).astype(int)
    ...
    def next(self):
        ...
        # size = self.V
        # aptRaw: series = self.params.baseAPT / ratio.apply(lambda x: math.pow(
...
class ATR_Rope(BtIndicator):
    ...
    def rope_smoother(self, _src: series):
        size = self.V
...
class BERLIN_MAX_1V5(BtIndicator):
    ...
    def next(self):
        ...
        ema = series(self.close.ewm(span=1, adjust=False).mean())
...
class Neural_Network_Buy_and_Sell_Signals(BtIndicator):
    ...
    def next(self):
        ...
        signalLow = self.low
        buy_signal_longStop: series = (signalClose.tqfunc.hhv(
        ...
        buy_signal_shortStop: series = signalClose.tqfunc.llv(
        ...
        size = self.V
        buy_signal_dir = series(np.ones(size))
        other = series(-np.ones(size))
        other = other.where(
            signalClose < buy_signal_longStopPrev, buy_signal_dir)
        buy_signal_dir: series = buy_signal_dir.where(
        ...
        sell_signal_longStop: series = signalClose.tqfunc.hhv(
        ...
        sell_signal_shortStop: series = signalClose.tqfunc.llv(
        ...
        sell_signal_dir = series(np.ones(size))
        other = series(-np.ones(size))
        other = other.where(
...
class RSI_ADX_Long_Short_Strategy_v6(BtIndicator):
    ...
    def next(self):
        ...
        plusDM: series = upMove.where((upMove > downMove) & (upMove > 0), 0.)
        minusDM: series = downMove.where(
...
class RCI_Ribbon(BtIndicator):
    ...
    def next(self):
        rci1: series = self.close.diff(self.params.shortLength).rolling_apply(
