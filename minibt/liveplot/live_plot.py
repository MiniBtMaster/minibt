                    ...
                    if doubles:
                        ...
                        if any(isplot):
                            ...
                            if is_overlay and not is_candles:  # 主图叠加
                                ...
                                source[i][j].add(_ohlc_inc, _ohlc_inc_field)
                                ...
                                # [修改] 支持“主图双蜡烛”: 蜡烛型指标 overlap=True 时画到**主图** fig_ohlc,
                                #   并用更窄柱宽(BAR_WIDTH/2)避免完全盖住主 K 线; overlap=False 仍开副图。
                                if is_overlay:
                                    fig_ohlc_ = fig_ohlc
                                ...
                                ohlc_bars_ = fig_ohlc_.vbar('index', _candle_w, names[filed_index[0]], names[filed_index[3]], source=source[i][j],
                                ...
                                # [修改] is_overlay 时 fig_ohlc_ 就是主图, 不能改其 y 轴标题/可见性
                                if not is_overlay:
                                ...
                                # [修改] 主图叠加时不往 indicator_figs 里加(它就是主图)
                                if not is_overlay:
