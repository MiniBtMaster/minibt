...
with FilteredOutputRedirector():
    from qfluentwidgets import (setTheme, Theme, FluentWindow, TransparentToolButton, FluentIcon,
                                FluentStyleSheet, FluentTitleBar, CaptionLabel,
                                BodyLabel, ComboBox, SegmentedWidget, SingleDirectionScrollArea,
                                PushButton, PrimaryPushButton, Slider, HorizontalSeparator,
                                TableWidget, CardWidget, FlowLayout)
from PySide6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout,
                               QSizePolicy, QStackedWidget, QTableWidgetItem,
                               QAbstractItemView, QScrollArea)
from PySide6.QtGui import QColor, QFont, QIcon, QGuiApplication, QPainter, QPen, QPolygonF
from PySide6.QtCore import QPointF
from PySide6.QtCore import Qt, QSize, QTimer, QUrl, QObject, Signal
...
def get_chart_class():
    ...
    if ChartClass is None:
        ...
        lightweight_charts.using_pyside6 = True
        ...
        lightweight_charts.widgets.Slot = Slot
...
class ReplayInfoWindow(QWidget):
    ...
    speed_changed = Signal(int)        # 新的速度(ms)
...
class ReplayTimer(QObject):
    ...
    step_completed = Signal(int, int)      # (current_index, total_candles)
    replay_finished = Signal()             # 回放完成
...
class ReplayWindow(QWidget):
    ...
    def isdark(self) -> bool:
        if self.main_window and hasattr(self.main_window, 'isdark'):
    ...
    def _fill_chart_segments(self, s_idx: int):
        ...
        try:
            ...
            self.chart_seg.adjustSize()
            # 确保最小高度，防止 clear 后高度收缩为 0
            min_height = max(self.chart_seg.height() + 4, 28)
            self.chart_seg_scroll.setFixedHeight(min_height)
            # 恢复信号
