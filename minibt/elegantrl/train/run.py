...
def train_agent(args: Config, if_single_process: bool = False):
    if if_single_process:
        print(
            f"| train_agent_single_process() with GPU_ID {args.gpu_id}", flush=True)
        return train_agent_single_process(args)
    elif len(args.learner_gpu_ids) == 0:
        print(
            f"| train_agent_multiprocessing() with GPU_ID {args.gpu_id}", flush=True)
        train_agent_multiprocessing(args)
    elif len(args.learner_gpu_ids) != 0:
        print(
            f"| train_agent_multiprocessing_multi_gpu() with GPU_ID {args.learner_gpu_ids}", flush=True)
        train_agent_multiprocessing_multi_gpu(args)
    else:
        ValueError(
            f"| run.py train_agent: args.learner_gpu_ids = {args.learner_gpu_ids}")
...
def train_agent_single_process(args: Config):
    ...
    if args.continue_train and args.train:
    ...
    eval_env = args.env_class
    evaluator = Evaluator(cwd=args.cwd, env=eval_env,
                          args=args, if_tensorboard=False)
    ...
    while if_train:
        ...
        if agent.if_swa:
            agent.current_epoch_progress = evaluator.total_step / break_step

    print(
        f'| UsedTime: {time.time() - evaluator.start_time:>7.0f} | SavedDir: {cwd}', flush=True)
...
def train_agent_multiprocessing(args: Config):
    ...
    '''build the Pipe'''
    worker_pipes = [Pipe(duplex=False)
                    for _ in range(args.num_workers)]  # receive, send
    learner_pipe = Pipe(duplex=False)
    evaluator_pipe = Pipe(duplex=True)
    '''只传递必要的配置参数，不传递可能包含锁的对象'''
    # args_dict = args.to_dict()  # 新增：将配置转换为字典
    '''build Process'''
...
def train_agent_multiprocessing_multi_gpu(args: Config):
    ...
    for gpu_id in args.learner_gpu_ids:
        ...
        learner = Learner(learner_pipe=learner_pipe,
                          ...
                          args=args_dict)
        workers = [Worker(worker_pipe=worker_pipe, learner_pipe=learner_pipe, worker_id=worker_id, args=args_dict)
                   for worker_id, worker_pipe in enumerate(worker_pipes)]
        evaluator = EvaluatorProc(
            evaluator_pipe=evaluator_pipe, args=args_dict)
...
class Learner(Process):
    def __init__(
            ...
            evaluator_pipe: Pipe,
            learners_pipe: list[Pipe] | None = None,
            args: Config = Config(),
    ):
        super().__init__()
    ...
    def run(self):
        ...
        learner_id = args.learner_gpu_ids.index(
            args.gpu_id) if len(args.learner_gpu_ids) > 0 else 0
        ...
        if len(args.learner_gpu_ids) >= 2:
            ...
            ValueError(
                "| Learner: suggest to set `args.learner_gpu_ids=()` in default")
        ...
        agent.last_state = th.empty(
            (num_seqs, state_dim), dtype=th.float32, device=agent.device)

        states = th.zeros((horizon_len, num_seqs, state_dim),
                          dtype=th.float32, device=agent.device)
        ...
        rewards = th.zeros((horizon_len, num_seqs),
                           dtype=th.float32, device=agent.device)
        undones = th.zeros((horizon_len, num_seqs),
                           dtype=th.bool, device=agent.device)
        unmasks = th.zeros((horizon_len, num_seqs),
                           dtype=th.bool, device=agent.device)
        if if_off_policy:
            buffer_items_tensor = (states, actions, rewards, undones, unmasks)
        else:
            logprobs = th.zeros((horizon_len, num_seqs),
                                dtype=th.float32, device=agent.device)
            buffer_items_tensor = (
        ...
        while if_train:
            ...
            for _ in range(num_workers):
                ...
                for buffer_item, buffer_tensor in zip(buffer_items, buffer_items_tensor):
                    buffer_tensor[:, buf_i:buf_j] = buffer_item.to(
                        agent.device)
            ...
            _buffer_items_tensor = [
                t[:, :_buffer_len].cpu().detach_() for t in buffer_items_tensor]
            ...
            for shift_id in range(num_communications):
                _learner_id = (learner_id + shift_id +
                               1) % num_learners  # other_learner_id
                ...
                for buffer_item, buffer_tensor in zip(_buffer_items_tensor, buffer_items_tensor):
                    buffer_tensor[:, _buf_i:_buf_j] = buffer_item.to(
                        agent.device)
            ...
            if if_train:
                ...
                exp_r = buffer_items_tensor[2].mean().item()
        ...
        if if_save_buffer and hasattr(buffer, 'save_or_load_history'):
            print(
                f"| LearnerPipe.run: ReplayBuffer saving in {cwd}", flush=True)
            buffer.save_or_load_history(cwd, if_save=True)
            print(
                f"| LearnerPipe.run: ReplayBuffer saved  in {cwd}", flush=True)
...
class EvaluatorProc(Process):
    ...
    def run(self):
        ...
        cwd = args.cwd
        break_step = args.break_step
        device = th.device(f"cuda:{args.gpu_id}" if (
            th.cuda.is_available() and (args.gpu_id >= 0)) else "cpu")
        # del args
        ...
        while self.pipe1.poll():  # whether there is any data available to be read of this pipe
            while self.pipe0.poll():
                try:
                    ...
                    print(
                        "| Evaluator Ignore RuntimeError in self.pipe0.recv()", flush=True)
                time.sleep(1)
            time.sleep(1)
