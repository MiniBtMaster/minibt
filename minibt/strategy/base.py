...
from ..indicators import (KLine, Base,
                          Addict, pd, Cache, KLineType,
...
from ..utils import (cache, time_to_str, Union, np, Callable,
                     ...
                     abc, time_to_datetime, timedelta,
                     TargetPosTask, KLinesSet, time, datetime,
...
if TYPE_CHECKING:
    ...
    from ..utils import TqApi, BtAccount, datetime, BtPosition, Position, TqObjs
...
class StrategyBase:
    ...
    def _get_plot_datas(self):
        ...
        if self.config.isplot:  # 仅当配置开启绘图时执行
            ...
            if self._strategy_replay:
                ...
                kline_datas = [
                    data.source_object for data in self._btklinedataset.values()]
            self._plot_datas = [
    ...
    def _execute_live_trading(self) -> Optional[bool]:
        ...
        sid = loadData(self._id_dir)
        # 当前策略即时更新
        # 非当前策略按合约周期更新
        cond = (sid == self._sid) or self.is_key_changing("datetime")
        if cond:
            self._isinit = False
        ...
        if self.rl:
            ...
            with self.th.no_grad():
                list(map(self.__process_rl_backtest_iteration, range(
                    self.btindex+1, self._btklinedataset.max_length)))
    ...
    def buy(self, data: KLine = None, size: int = 1, stop=None, **kwargs):
        """
        买入开仓/加仓接口（统一封装回测与实盘逻辑）
        支持指定合约、手数、止损参数，自动校验手数有效性

        Args:
            data (KLine, optional): 目标合约数据（默认使用默认合约_btklinedataset.default_kline）. Defaults to None.
    ...
    def _buy_back_trading(self, data: KLine = None, size: int = 1, stop=None, **kwargs) -> float | None:
        """
        ...
        # 设置止损（若未启用且指定stop参数）
        if not data._klinesetting.isstop:
            if stop and data._klinesetting.stop is None:
    ...
    def sell(self, data: KLine = None, size: int = 1, stop=None, **kwargs) -> None:
        """
    ...
    def _sell_back_trading(self, data: KLine = None, size: int = 1, stop=None, **kwargs):
        """
    ...
    def kline(self) -> KLine:
        """
        获取默认K线数据（属性接口）
        返回数据集合中的第一个KLine对象，适用于单合约策略

        Returns:
            KLine: 默认K线数据对象
        """
        """### 默认KLine"""
        return self._btklinedataset[0]
    ...
    def data_from_csv(self, path: str) -> KLine:
        ...

    def data_from_dataframe(self, data: pd.DataFrame) -> KLine:
        ...
    ...
    def data_from_pytdx(self) -> KLine:
        ...
    ...
    def data_from_locals(self) -> KLine:
        ...

    def data_random(self,) -> KLine:
        ...
