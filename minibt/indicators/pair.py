...
class Pair:
    ...
    - `vecm_based`: VECM模型策略 - 基于向量误差修正模型识别长期均衡关系
    ...
    def kalman_filter(self, y_series=None, window=60,state_var_init=1e-4,obs_var=1e-2, **kwargs) -> IndFrame:
        """
        ## 卡尔曼滤波配对交易指标
        ...
        **检验方法建议：**
        ...
        ----------
        self : IndSeries
            leg[0] 价格序列（x_series），即主图对应的资产。

        y_series : IndSeries
            leg[1] 价格序列，与之配对的另一个资产。
        ...
        --------
        ```python
        # 两个资产的价格序列，类型 IndSeries
        # self = leg[0] 价格，y_series = leg[1] 价格
        result = self.kalman_filter(y_series)
