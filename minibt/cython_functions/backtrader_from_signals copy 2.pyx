...
# ===== 配对交易辅助函数 =====
cdef inline void _init_sl_tp(int j, double trade_price,
                              list sl_price, list tp_price,
                              list sl_ref_price, list sl_distance, list tp_distance,
                              list direction, list current_size,
                              list price_tick_list, list volume_multiple_list,
                              double sl_stop, double tp_stop, int stop_mode,
                              int use_sl_cb, int use_tp_cb):
    """为新建仓的合约初始化止损/止盈价格。"""
...
cdef inline void _exit_position(
    int j, double trade_price, double com,
    list direction, list entry_price, list current_size,
    list volume_multiple_list,
    list pnl, list position, list in_position,
    list current_margin,
    list sl_price, list tp_price, list sl_ref_price,
    list hold_bars,
    double* cum_profit, double* current_cash,
    double* total_equity, double* total_fee
):
    """平仓合约 j 的持仓（市价平仓），更新所有状态变量。
    由配对交易的级联平仓（Phase 3）调用。"""
    cdef double profit
    cdef double vm = volume_multiple_list[j]
    cdef double ep = entry_price[j]
    cdef double sz = current_size[j]
    cdef double marg = current_margin[j]

    if direction[j] == 1:
        profit = (trade_price - ep) * sz * vm - com
    else:
        profit = (ep - trade_price) * sz * vm - com

    pnl[j] = profit
    cum_profit[0] += profit
    current_cash[0] += marg + profit
    current_margin[j] = 0.0
    total_equity[0] += profit
    total_fee[0] += com
    position[j] = 0.0
    in_position[j] = 0
    direction[j] = 0
...
def from_signals(
    ...
    # ===== 配对交易参数 =====
    object pairs=None,              # [(0,1), (2,3)] 合约0和1配对，2和3配对
    object pair_size_ratios=None,   # [(1.0, 2.0), ...] 每条腿的手数比例，默认1:1
):
    cdef int i, j
    ...
    # 处理参数，转换为与合约长度相同的列表
    # 辅助函数：将标量或列表统一扩展为长度为 m 的列表
    # 处理size参数
    if isinstance(size, (int, float)):
        size_list = [float(size) for _ in range(m)]
    elif len(size) == 1 and m > 1:
        size_list = [float(size[0]) for _ in range(m)]
    else:
    ...
    # ===== 解析配对交易参数 =====
    cdef list pair_leg_a = []
    cdef list pair_leg_b = []
    cdef list pair_ratio_a = []
    cdef list pair_ratio_b = []
    cdef int num_pairs = 0
    cdef int pair_jj, a_idx, b_idx
    cdef double r_a, r_b

    if pairs is not None:
        num_pairs = len(pairs)
        if pair_size_ratios is not None:
            assert len(pair_size_ratios) == num_pairs, "pair_size_ratios长度必须与pairs一致"
        for pair_jj in range(num_pairs):
            a_idx, b_idx = pairs[pair_jj]
            pair_leg_a.append(a_idx)
            pair_leg_b.append(b_idx)
            if pair_size_ratios is not None:
                r_a, r_b = pair_size_ratios[pair_jj]
            else:
                r_a, r_b = 1.0, 1.0
            pair_ratio_a.append(r_a)
            pair_ratio_b.append(r_b)

    # 反向索引：每个合约属于哪个pair（-1=不属于任何pair）
    cdef list pair_map = [-1 for _ in range(m)]
    for pair_jj in range(num_pairs):
        pair_map[pair_leg_a[pair_jj]] = pair_jj
        pair_map[pair_leg_b[pair_jj]] = pair_jj
    
    # ===== 配对交易临时变量（per-bar）=====
    cdef list pair_entered_this_bar
    cdef int did_pair_close_a, did_pair_close_b
    cdef double p_trade_price, p_com
    cdef double trade_price_a, trade_price_b
    cdef double sz_a, sz_b
    cdef double com_a, com_b
    ...
    for i in range(n):
        ...
        # ===== Phase 1: 配对入场预检（联合资金校验，原子入场）=====
        pair_entered_this_bar = [0 for _ in range(m)]
        if num_pairs > 0:
            for pair_jj in range(num_pairs):
                a_idx = pair_leg_a[pair_jj]
                b_idx = pair_leg_b[pair_jj]

                # 前置条件：两条腿都空仓，且都有信号，且方向相反
                if (not in_position[a_idx] and not in_position[b_idx]
                    and entries[i, a_idx] != 0 and entries[i, b_idx] != 0
                    and entries[i, a_idx] == -entries[i, b_idx]):

                    direction[a_idx] = <int>entries[i, a_idx]
                    direction[b_idx] = <int>entries[i, b_idx]

                    # 应用滑点
                    if slip_point > 0:
                        if direction[a_idx] == 1:
                            trade_price_a = prices[i, a_idx] + slip_point
                        else:
                            trade_price_a = prices[i, a_idx] - slip_point
                    else:
                        trade_price_a = prices[i, a_idx]

                    if slip_point > 0:
                        if direction[b_idx] == 1:
                            trade_price_b = prices[i, b_idx] + slip_point
                        else:
                            trade_price_b = prices[i, b_idx] - slip_point
                    else:
                        trade_price_b = prices[i, b_idx]

                    # 计算手数（使用入场前 current_cash，两条腿共享同一资金池）
                    sz_a = calculate_size(trade_price_a, size_list[a_idx], size_type,
                                          current_cash, margin_rate_list[a_idx],
                                          volume_multiple_list[a_idx])
                    sz_b = calculate_size(trade_price_b, size_list[b_idx], size_type,
                                          current_cash, margin_rate_list[b_idx],
                                          volume_multiple_list[b_idx])

                    # 应用配对手数比例
                    sz_a *= pair_ratio_a[pair_jj]
                    sz_b *= pair_ratio_b[pair_jj]
                    sz_a = float(max(<int>sz_a, 1))
                    sz_b = float(max(<int>sz_b, 1))

                    # 计算手续费和保证金
                    com_a = calculate_commission(trade_price_a, sz_a, volume_multiple_list[a_idx],
                                                 commission_list[a_idx], com_type, price_tick_list[a_idx])
                    com_b = calculate_commission(trade_price_b, sz_b, volume_multiple_list[b_idx],
                                                 commission_list[b_idx], com_type, price_tick_list[b_idx])
                    margin_a = trade_price_a * sz_a * volume_multiple_list[a_idx] * margin_rate_list[a_idx]
                    margin_b = trade_price_b * sz_b * volume_multiple_list[b_idx] * margin_rate_list[b_idx]
                    total_required = margin_a + com_a + margin_b + com_b

                    # 联合资金检查：要么两条腿都入场，要么都不入场
                    if current_cash >= total_required:
                        # ---- 原子入场：合约 A ----
                        current_size[a_idx] = sz_a
                        total_equity -= com_a
                        current_cash -= com_a + margin_a
                        current_margin[a_idx] = margin_a
                        total_fee += com_a
                        cum_profit -= com_a
                        position[a_idx] = direction[a_idx]
                        entry_price[a_idx] = trade_price_a
                        in_position[a_idx] = 1
                        hold_bars[a_idx] = 0
                        _init_sl_tp(a_idx, trade_price_a,
                                    sl_price, tp_price, sl_ref_price, sl_distance, tp_distance,
                                    direction, current_size,
                                    price_tick_list, volume_multiple_list,
                                    sl_stop, tp_stop, stop_mode,
                                    use_sl_cb, use_tp_cb)

                        # ---- 原子入场：合约 B ----
                        current_size[b_idx] = sz_b
                        total_equity -= com_b
                        current_cash -= com_b + margin_b
                        current_margin[b_idx] = margin_b
                        total_fee += com_b
                        cum_profit -= com_b
                        position[b_idx] = direction[b_idx]
                        entry_price[b_idx] = trade_price_b
                        in_position[b_idx] = 1
                        hold_bars[b_idx] = 0
                        _init_sl_tp(b_idx, trade_price_b,
                                    sl_price, tp_price, sl_ref_price, sl_distance, tp_distance,
                                    direction, current_size,
                                    price_tick_list, volume_multiple_list,
                                    sl_stop, tp_stop, stop_mode,
                        ...
                        # 标记本 bar 已由配对入场处理
                        pair_entered_this_bar[a_idx] = 1
                        pair_entered_this_bar[b_idx] = 1
        ...
        for j in range(m):
            ...
            # ===== 检查入场信号 =====
            # 配对入场已在 Phase 1 处理，此处跳过
            if not pair_entered_this_bar[j] and not in_position[j] and entries[i, j] != 0:
        ...
        if num_pairs > 0:
            for pair_jj in range(num_pairs):
                ...
                # 一条腿已平仓，另一条还在持仓 → 级联平仓
                if in_position[a_idx] and not in_position[b_idx]:
                    # 以当前 bar 市价平掉 A
                    if slip_point > 0:
                        if direction[a_idx] == 1:
                            p_trade_price = prices[i, a_idx] - slip_point
                        else:
                            p_trade_price = prices[i, a_idx] + slip_point
                    else:
                        p_trade_price = prices[i, a_idx]
                    p_com = calculate_commission(p_trade_price, current_size[a_idx],
                                                 volume_multiple_list[a_idx], commission_list[a_idx],
                                                 com_type, price_tick_list[a_idx])
                    _exit_position(a_idx, p_trade_price, p_com,
                                  direction, entry_price, current_size, volume_multiple_list,
                                  pnl, position, in_position, current_margin,
                                  sl_price, tp_price, sl_ref_price, hold_bars,
                                  &cum_profit, &current_cash, &total_equity, &total_fee)
                    # 更新级联平仓合约的结果记录
                    result[a_idx][i, 0] = total_equity
                    result[a_idx][i, 1] = position[a_idx]
                    result[a_idx][i, 2] = current_size[a_idx]
                    result[a_idx][i, 3] = pnl[a_idx]
                    result[a_idx][i, 4] = cum_profit
                    result[a_idx][i, 5] = total_fee

                elif not in_position[a_idx] and in_position[b_idx]:
                    # 以当前 bar 市价平掉 B
                    if slip_point > 0:
                        if direction[b_idx] == 1:
                            p_trade_price = prices[i, b_idx] - slip_point
                        else:
                            p_trade_price = prices[i, b_idx] + slip_point
                    else:
                        p_trade_price = prices[i, b_idx]
                    p_com = calculate_commission(p_trade_price, current_size[b_idx],
                                                 volume_multiple_list[b_idx], commission_list[b_idx],
                                                 com_type, price_tick_list[b_idx])
                    _exit_position(b_idx, p_trade_price, p_com,
                                  direction, entry_price, current_size, volume_multiple_list,
                                  pnl, position, in_position, current_margin,
                                  sl_price, tp_price, sl_ref_price, hold_bars,
                                  &cum_profit, &current_cash, &total_equity, &total_fee)
                    # 更新级联平仓合约的结果记录
                    result[b_idx][i, 0] = total_equity
                    result[b_idx][i, 1] = position[b_idx]
                    result[b_idx][i, 2] = current_size[b_idx]
                    result[b_idx][i, 3] = pnl[b_idx]
                    result[b_idx][i, 4] = cum_profit
                    result[b_idx][i, 5] = total_fee
