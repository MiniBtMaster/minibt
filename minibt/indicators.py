...
from .utils import \
    (TYPE_CHECKING, PlotInfo, np, pd, wraps, Union, Callable, BtID, retry_with_different_params,
     ...
     PandasDataFrame, PandasSeries, signature, SignalLabel, ExponentialMovingWindow, Orders,
...
def set_lines(btind: Union[KLine, IndFrame]):
    """多维指标数据更改后,内置Line数据同步更改"""
    # 确保 btind 有 pandas_object 属性
    if not hasattr(btind, 'pandas_object'):
        return

    pandas_obj = btind.pandas_object

    # 确保 pandas_obj 有 values 属性
    if not hasattr(pandas_obj, 'values'):
        return

    data = pandas_obj.values

    # 确保有 line_filed 属性
    if not hasattr(btind._indsetting, 'line_filed'):
        return

    # 遍历所有 line 字段并更新对应的 Line 对象
    for i, line_field in enumerate(btind._indsetting.line_filed):
        # 确保 line_field 存在
        if hasattr(btind, line_field):
            line_obj = getattr(btind, line_field)
            ...
            if i < data.shape[1]:
                ...
                # 使用更安全的方式更新 Line 对象
                # 方法1：如果 line_obj 是 IndSeries，使用 .values 属性
                # if hasattr(line_obj, 'values'):
                #     # 确保 column_data 的形状正确
                #     if len(column_data.shape) == 1:
                #         # 使用 np.copyto 直接复制数据
                #         np.copyto(line_obj.values, column_data)
                #     elif len(column_data.shape) == 2 and column_data.shape[1] == 1:
                #         # 如果是二维数组且只有一列，展平
                #         np.copyto(line_obj.values, column_data.flatten())

                # 方法2：使用 pandas 的赋值操作
                line_obj.iloc[:] = column_data
...
class MinibtIndexerBase:
    ...
    def _is_kline_data(self, pandas_obj: pd.DataFrame) -> bool:
        ...
        """
        try:
            # return all(field in pandas_obj.columns for field in FILED.ALL)
...
def tobtind(lines: list[str] | dict[str, bool] | None = None, overlap: dict | bool = False,
    ...
    **kwargs_:
    ...
    _sname = kwargs_.pop("sname", "")          # 简写名称
    # _ind_name = kwargs_.pop("ind_name", "")    # 指标名称
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs) -> Union[IndFrame, IndSeries]:
            ...
            # 5. 处理各种参数（允许覆盖装饰器默认值）
            sname = kwargs.pop("sname", _sname or func_name)  # 简写名（优先使用传入的）
...
class IndicatorsBase(Base):
    ...
    def __getitem__(self, key) -> KLine | IndFrame | IndSeries | Line:
        ...
        # data = None
        # try:
        #     data = self.pandas_object.__getitem__(key)

        # except:
        #     try:
        #         data = self.pandas_object.iloc[key]
        #     except:
        #         try:
        #             data = self.pandas_object.loc[key]
        #         except:
        #             ...
        # if data is None:
        #     raise KeyError(f"{key}参数有误")
    ...
    def __setitem__(self, key, value):
        ...
        #     # 元组key，使用loc进行赋值（loc能处理(slice, list)这种情况）
        #     super().loc[key] = value
        #     self.pandas_object.loc[key] = value
        # else:
        #     # 非元组key，直接调用父类的__setitem__
        super().__setitem__(key, value)
        self.pandas_object.__setitem__(key, value)
        # self._dataset.pandas_object = self.copy()
    ...
    def _update_line_data(self, data: KLine | IndFrame | IndSeries | np.ndarray | None = None):
        """更新 Line 对象的数据"""
        if data is None:
            data = self
        ...
        if hasattr(data, "values"):
            data = data.values
        for i, line_field in enumerate(self._indsetting.line_filed):
            line_obj: Line = getattr(self, line_field)
            # line_obj._set_object(values[:, i])
            line_obj[:] = data[:, i]
...
class Pair:
    ...
    def __init__(self, data):
        ...
        Parameters
        ----------
        data : IndFrame | IndSeries
            配对交易数据：
            - 如果传入IndSeries：直接作为价差序列（spread_series）使用
            - 如果传入IndFrame：包含两只相关资产的收益率或价格序列
              - 通常第一列为被解释变量（y）
              - 第二列为解释变量（x）

        Notes
        -----
        配对交易要求两只资产具有长期协整关系，常用的协整检验方法包括：
...
class Factors:
    ...
    def __init__(self, data):
        ...
        ----------
        data : IndFrame | IndSeries
            输入数据，通常是价格序列或已计算好的因子矩阵
            - 如果为IndSeries：价格序列，用于后续计算因子
            - 如果为IndFrame：因子矩阵，行代表时间，列代表不同因子

        Notes
        -----
        多因子模型是现代量化投资的核心，通过组合多个独立的预测信号，
        可以提高预测准确性和策略稳定性。常见因子包括：
        - 技术因子：移动平均、RSI、MACD等
        - 基本面因子：市盈率、市净率、股息率等
        - 宏观因子：利率、通胀率、GDP增长率等
        """
...
class CoreIndicators:
    ...
    def __init__(self, data):
        ...
        Parameters
        ----------
        data : IndFrame | IndSeries | pd.DataFrame | pd.Series
            输入数据，可以是：
            - 价格序列（如收盘价、开盘价等）
            - 包含多列价格数据的DataFrame（如OHLC数据）
            - 已经计算好的指标序列

        Notes
        -----
        1. 输入数据会自动转换为Pandas对象进行处理
        2. 对于时间序列数据，确保索引是时间类型
        3. 如果输入包含NaN值，某些指标可能会产生NaN结果
        4. 不同指标库对数据格式要求可能不同，该类会自动处理转换
        """
        self._df = data
...
class IndFrame(IndicatorsBase, PandasDataFrame, PandasTa, TaLib):
    ...
    # 特殊函数
    def select_dtypes(self, include=None, exclude=None, **kwargs) -> IndFrame | IndSeries:
        """根据数据类型选择列
        ...
        Args:
            include: 包含的数据类型，可以是：
                - 字符串：'number', 'integer', 'floating', 'bool', 'object', 
                        'datetime', 'timedelta', 'category', 'complex'
                - NumPy dtype：np.int32, np.float64, np.bool_, np.datetime64
                - Python类型：int, float, bool, str, datetime.datetime
                - pandas扩展类型：'Int64', 'Float64', 'boolean', 'string'
                - 列表：以上类型的任意组合
            exclude: 排除的数据类型，格式同include
    ...
    def pop(self, item: Hashable, **kwargs) -> IndSeries:
        ...
        if not options.check_conversion_mode(pandas_result, self):
            return pandas_result
        indicator_kwargs = self.get_indicator_kwargs()
        indicator_kwargs["lines"] = list(self.pandas_object.columns)
        data = IndFrame(self.pandas_object.values, **indicator_kwargs)
        self.__dict__.update(data.__dict__)
        indicator_kwargs["lines"] = [item,]
        indicator_kwargs["ind_name"] = item
        return IndSeries(pandas_result.values, **indicator_kwargs)
...
class KLine(IndFrame):
    ...
    def set_target_size(self, size: int = 0):
        """
        ## 通过KLine对象设置目标仓位
