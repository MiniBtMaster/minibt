...
class UT_Bot_Alerts(BtIndicator):
    ...
    def next(self):
        ...
        # 详细调试信息
        print("=== 详细调试 ===")
        print(f"close[-5:]: {close.values[-5:]}")
        print(f"xATR[-5:]: {xATR.values[-5:]}")
        print(f"nLoss[-5:]: {nLoss.values[-5:]}")
        print(f"up[-5:]: {up[-5:]}")
        print(f"dn[-5:]: {dn[-5:]}", len(dn), type(dn[-1]))
        print(f"close.dtype: {close.dtype}")
        print(f"xATR.dtype: {xATR.dtype}")
        print(f"nLoss.dtype: {nLoss.dtype}")

        # 手动计算验证
        manual_up = close.values[-5:] + nLoss.values[-5:]
        manual_dn = close.values[-5:] - nLoss.values[-5:]
        print(f"手动计算 up: {manual_up}")
        print(f"手动计算 dn: {manual_dn}")
        # print(close.values.tolist())
        # print(xATR.values.tolist())
        # print(up.tolist())
        # print(dn.tolist())
        src = close.values
...
class TradingView:
    ...
    """
    _kline: Union[KLine, IndFrame, IndSeries]
    ...
    def __init__(self, data):
        ...
        Args:
            data: 输入数据，可以是KLine、IndFrame或IndSeries格式的价格数据
        """
        self._kline = data
