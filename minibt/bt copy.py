...
from .strategy.strategy import Strategy, StrategyMeta
import os
...
class Bt:
    ...
    def __init__(self, auto=True, **kwargs) -> None:
        """
        初始化回测框架实例

        Args:
            auto: 是否自动加载全局资源（调试用，生产环境建议手动添加）
                  - True：自动查找全局DataFrame数据和TqApi实例
                  - False：仅手动添加资源
            kwargs: 额外配置参数
                data_dir: 数据存储目录路径（str/None，默认为None）
                quick_live: 快速实盘配置（dict，含'live'键控制是否进入实盘模式）

        初始化核心属性：
        - 运行状态：记录启动时间、回测完成状态、优化模式开关
        - 数据相关：存储回测数据、数据目录
        - 策略相关：存储策略类、策略参数
        - 实盘相关：TqApi实例、实盘模式开关、期货合约字典
        """
        ...
        self.__online: bool = False  # 在线模式标记（预留）
        self._api = None  # 天勤TqApi实例（实盘/模拟盘连接，初始未初始化）
        self.__contracts: list[str] | dict = []  # 期货合约存储（初始空）
        self.__sqlite = None  # SQLite数据库连接（预留，未使用）
        self.__live: bool = False  # 是否进入实盘模式（初始关闭）
        self.__contracts: dict = {}  # 期货合约字典（key：简称，value：全称，实盘用）
        self.__params: list = []  # 策略参数列表（与策略类一一对应）
        self.__final_merged_output = None

        # 解析数据目录参数（校验合法性）
        data_dir = kwargs.pop('data_dir', None)
        self.__data_dir: str = data_dir if isinstance(
            data_dir, str) and data_dir else None
    ...
    def __tq_real(self, isplot: bool, **kwargs) -> None:
        ...
        # 解析实盘参数
        black_sytle = kwargs.pop('black_sytle', False)  # 画图是否黑色主题（默认白色）
        ...
        # _tq_contracts_dict = self.__contracts  # 期货合约字典
        name = [s.__name__ for s in self.strategy]  # 策略名称列表
        # 初始化队列（用于策略进程与画图进程间通信）
        datas_queue = Queue(maxsize=contracts_num)  # 行情数据队列
        trade_queue = Queue(maxsize=contracts_num)  # 交易数据队列
        ...
        if isplot:
            ...
            if plot_width:
                cmds_string = f'python {BASE_DIR}/liveplot/live_plot.py -bs {black_sytle} -pw {plot_width} -pm {period_milliseconds} -cp {click_policy}'
            else:
                cmds_string = f'python {BASE_DIR}/liveplot/live_plot.py -bs {black_sytle} -pm {period_milliseconds} -cp {click_policy}'
        ...
        while True:
            ...
            if isplot:
                ...
                if datas_queue.full():
                    ...
                    # storeData(
                    # list(map(lambda x: x[1], sorted_datas)), update_datas_dir)
                ...
                if trade_queue.full():
                    ...
                    # storeData(
                    #     list(map(lambda x: x[1], sorted_trades)), update_trade_dir)
    ...
    def run(self, isplot=True, isreport: bool = False, **kwargs) -> Bt:
        """策略启动入口（根据当前模式自动执行：实盘/参数优化/回测）
        ---

        ### Args:
        >>> isplot (bool, optional): 回测/实盘是否生成图表. Defaults to True.
            isreport (bool, optional): 是否生成QuantStats回测分析报告. Defaults to False.

        ### Kwargs:
        >>> # 多策略回测参数
            model (str): 多线程库（可选['dask','joblib','sklearn','multiprocessing']）. Defaults to 'joblib'.
            # 图表参数
            trade_signal (bool): 是否显示交易信号（开仓/平仓标记）. Defaults to True.
            black_sytle (bool): 图表是否黑色主题（默认白色）. Defaults to False.
            plot_width (int): 图表宽度（像素，默认全屏）. Defaults to None.
            period_milliseconds (int): 实盘数据更新频率（毫秒）. Defaults to 0.
            open_browser (bool): Jupyter Notebook中是否用浏览器打开图表. Defaults to False.
            save_plot (bool): 是否保存图表文件（HTML格式）. Defaults to True.
            # 账户参数
            print_account (bool): 是否打印账户信息. Defaults to False.
    ...
    def bokeh_plot(
            ...
            ** kwargs):
        """生成Bokeh交互式图表（展示回测结果：行情、交易信号、账户净值等）
        -------------

        Args:
        -------------
            select (Union[str,int,List[int]], optional): 选择策略（多策略时生效）
                                                         - 'all'：所有策略
                                                         - int：单个策略索引
                                                         - list[int]：多个策略索引
                                                         Defaults to 'all'.
            filename (str, optional): 图表保存文件名（不含后缀，默认'bokeh_plot'）. Defaults to ''.
            plot_width (int, optional): 图表宽度（像素，默认1500）. Defaults to 1500.
            black_sytle (bool, optional): 图表是否黑色主题（默认白色）. Defaults to True.
            notebook_handle (bool, optional): Jupyter Notebook中是否返回图表句柄. Defaults to False.
            open_browser (bool, optional): 是否在浏览器中打开图表（默认False）. Defaults to False.

        Returns:
            (List[List[DataFrame]] | list[DataFrame] | None): 返回回测结果DataFrame（行情、交易等数据）
        """
        ...
        # 调用绘图函数生成图表
        tabs = plot(self.strategy, trade_signal, black_sytle,
                    open_browser, plot_width, plot_cwd, plot_name, **kwargs)
    ...
    def qs_reports(self,
                   ...
                   **kwargs) -> "Bt":
        """
        生成QuantStats回测分析报告（合并多策略为单个HTML文件）
        修复了导航可见但内容不显示的问题
        """
        import os
        ...
        try:
            ...
            raise ImportError(
                "请先安装quantstats和beautifulsoup4：pip install quantstats beautifulsoup4")
        ...
        #             # print("📊 合并报告:")
        #             # display(FileLinks(final_report_dir, result_html_prefix="→ "))
    ...
    def _qs_reports(self,
        ...
        """
        生成QuantStats回测分析报告（合并多策略为单个HTML文件）
        核心功能：自动处理路径、校验文件生成、去重CSS/JS、添加导航栏，支持临时文件保留与调试

        Args:
            report_cwd (str): 报告保存根目录
                - 未指定时默认：当前脚本所在目录/analysis_reports
                Defaults to "".
            report_name (str): 合并后报告的基础名称（不含.html后缀）
                Defaults to "analysis_reports".
            show (bool): 合并完成后是否自动用浏览器打开报告
                Defaults to True.
            keep_temp (bool): 是否保留临时文件（用于调试单个策略报告）
                - True：保留临时目录（temp_xxx）
                - False：合并后自动删除临时目录
                Defaults to False.

        Returns:
            Bt: 自身实例（支持链式调用，如Bt().run().qs_reports()）

        Raises:
            ValueError: 未选择有效策略或无有效临时报告时抛出
            PermissionError: 目录无读写权限时抛出
        """
        import os
        IS_JUPYTER_NOTEBOOK = 'JPY_INTERRUPT_EVENT' in os.environ
        if len(self.strategy) == 1:
        ...
        for idx, strategy in enumerate(self.strategy):
            ...
            # 使用策略的_qs_reports方法生成报告
            try:
                # 直接调用策略的_qs_reports方法，让其生成到临时目录
                strategy._qs_reports(
                    report_cwd=temp_dir,  # 指定临时目录
                    report_name=f"strategy_{idx}_{strategy_name}",  # 指定报告名称
                    show=False,  # 不自动打开
                    **kwargs
                )
            except Exception as e:
            ...
            # 查找生成的HTML文件（可能有多个，找到最新的）
            # 注意：_qs_reports函数会在temp_dir下创建reports子目录，所以需要在这个子目录中查找
            actual_report_dir = os.path.join(temp_dir, "reports")
            if not os.path.exists(actual_report_dir):
                # print(f"| 跳过策略 {idx}（{strategy_name}）：未找到reports子目录")
                continue
            ...
            if not html_files:
                # print(f"| 跳过策略 {idx}（{strategy_name}）：未找到HTML文件")
                continue
            ...
            # 检查文件是否有效
            if os.path.getsize(temp_path) == 0:
                # print(f"| 跳过策略 {idx}（{strategy_name}）：临时文件为空 - {temp_path}")
                os.remove(temp_path)
                continue

            temp_report_paths.append((idx, strategy_name, temp_path))
            # print(f"| 已生成策略 {idx}（{strategy_name}）的临时报告：{temp_path}")

        if not temp_report_paths:
            raise ValueError("无有效临时报告可合并（所有策略报告生成失败）")
        # print(f"\n| 共生成 {len(temp_report_paths)} 个有效临时报告（临时目录：{temp_dir}）")

        # -------------------------- 5. 合并报告：提取CSS/JS/内容+去重 --------------------------
        merged_css = set()
        merged_js = set()
        merged_content = []
        nav_items = []
        ...
        for idx, strategy_name, temp_path in temp_report_paths:
            ...
            <div id="{strategy_anchor_id}" class="strategy-report" style="display: none;">
                ...
                <div class="strategy-content">{str(body_content)}</div>
            </div>
            """
            merged_content.append(strategy_content)
        ...
        optimized_css = f"""
        {final_css}
        /* 移除与 Notebook 冲突的全局样式，保留核心样式 */
        body {{
            font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
            background: transparent;  /* 继承 Notebook 背景，避免白色块 */
            color: #2d3748;
            margin: 0;
            padding: 0;
        }}
        ...
        /* 调整导航栏：从固定定位改为浮动，避免遮挡 Notebook 内容 */
        #report-nav {{
            float: left;
            width: 220px;
            margin-right: 20px;
        ...
        #             function scrollToStrategy(strategyId) {{
        #                 const strategyReports = document.querySelectorAll('.strategy-report');
        #                 strategyReports.forEach(report => report.classList.remove('active'));
        #                 const targetReport = document.getElementById(strategyId);
        #                 if (targetReport) {{
        #                     targetReport.classList.add('active');
        #                     window.scrollTo({{top: targetReport.offsetTop - 10, behavior: 'smooth'}});
        #                 }}
        #             }}
        #         }};
        #     </script>
        ...
        try:
            ...
            # 将合并报告移动到最终目录
            shutil.copy2(merged_output, final_merged_output)
            # print(f"| 合并报告已移动到最终目录：{final_merged_output}")
        ...
        if not IS_JUPYTER_NOTEBOOK:
            if not keep_temp:
                try:
                    # 清理临时目录
                    shutil.rmtree(temp_reports_dir)
                    # print(f"\n| 已清理临时目录：{temp_reports_dir}")
                except Exception as e:
                    print(f"| 清理临时目录失败：{str(e)}，请手动删除：{temp_reports_dir}")
            else:
                ...
                # print(f"\n| 临时目录已保留（用于调试）：{temp_reports_dir}")
        ...
        #             # 历遍temp_reports_dir下所有html,然后一个个读取
        #             # 读取保存的 HTML 文件内容
