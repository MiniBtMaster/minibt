...
[![Version](https://img.shields.io/badge/version-1.2.8-green.svg)](https://github.com/MiniBtMaster/minibt)
...
默认安装即为**全功能**（指标 / 回测 / 图表 / 参数优化 / 统计报告 / 实盘 / 强化学习 / 因子分析）：

```bash
pip install minibt
```

> 全部重依赖（PyQt6、torch、tqsdk、quantstats 等）都采用**按需惰性 import**：
> 依赖会一并安装，但 `import minibt` 不会加载它们，启动时间仍保持在 **~1.2s**。

开发 / 打包额外工具：

```bash
pip install "minibt[dev]"
```
...
mystrategy= MyStrategy.copy(params=dict(symbol="v2601_60"))
bt.addstrategy(mystrategy)  # 副本+参数
