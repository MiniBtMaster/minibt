...
class StrategyBase:
    ...
    @classmethod
    def copy(cls, name: str = '', **kwargs) -> Strategy:
        """
        复制策略类（创建新的策略类实例）
    ...
    def __qs_reports(self, report_cwd="", report_name="", show=False, **kwargs):
        """
        ...
        # 1. 构建报告保存路径
        report_cwd = report_cwd if report_cwd and isinstance(report_cwd, str) else os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "analysis_reports")

        report_dir = report_cwd if show else os.path.join(
            report_cwd, "reports")
        # 若目录不存在则创建
        ...
        if show:
            import webbrowser
            print(
                f"| Analysis reports save to: {os.path.normpath(os.path.join(report_cwd,filename))}")
            try:
                if kwargs.pop("IS_JUPYTER_NOTEBOOK"):
                    from IPython.display import display, HTML, FileLinks
                    import base64
                    output = os.path.abspath(output)
                    # 显示最终合并报告
                    # print("📊 报告:")
                    # display(FileLinks(output, result_html_prefix="→ "))
                    ...
                    try:
                        ...
                        display(HTML(f"""
                            <p>📊  分析报告:</p>
                            <!-- 浏览器渲染打开（默认显示页面效果） -->
                            <a href="{output}" target="_blank" style="display: inline-block; margin-right: 15px; padding: 8px 12px; background: #4CAF50; color: white; text-decoration: none; border-radius: 4px;">
    ...
    #     report_dir = report_cwd if (
    #         show and not IS_JUPYTER_NOTEBOOK) else os.path.join(report_cwd, "reports")
    ...
    #     # 4. 打印保存路径提示，自动打开报告（若show=True）
    #     if show:
    #         print(f"| Analysis reports save to: {os.path.normpath(output)}")
    ...
    #                 # 显示报告文件链接
    #                 print("📊 报告:")
    #                 display(FileLink(output, result_html_prefix="→ "))
