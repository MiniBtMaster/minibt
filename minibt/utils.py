...
from typing import Any, Mapping, Callable, Iterable, Union, Optional, Sequence, Generator, TYPE_CHECKING, overload, Hashable
...
if TYPE_CHECKING:
    ...
    from .indicators import KLine, IndFrame, IndSeries, Line, IndicatorsBase
    from .bt import Bt
...
class MinibtOptions:
    ...
    Examples:
        >>> # 全局设置
        >>> minibt.options.conversion_mode = 'strict'
        >>> minibt.options.conversion_mode = 'free'
    ...
    def __init__(self):
        self.conversion_mode = 'strict'

    def __setattr__(self, name: str, value: str):
        if name == 'conversion_mode':
            if value not in ['strict', 'free']:
                raise ValueError("conversion_mode must be 'strict' or 'free'")
        super().__setattr__(name, value)

    def check_conversion(self, result: Iterable, reference_length: int) -> bool:
        """判断是否应该将pandas对象转换为minibt指标

        Args:
            result: 计算结果对象
            reference_length: 参考长度（通常是原始K线数据长度）

        Returns:
            bool: 是否应该转换
        """
        if self.conversion_mode == 'free':
            return True
        return len(result) == reference_length
    ...
    def __repr__(self) -> str:
        return f"MinibtOptions(conversion_mode='{self.conversion_mode}')"
...
def __assign(self: IndFrame, **kwargs):
    # assign修改
...
class SignalStyle(DataSetBase):
    ...
    # def __setattr__(self, name, value):
    #     super().__setattr__(name, value)
    #     if name=="label" and isinstance(value,bool) and value:
    #         self.set_default_label(name)
...
class IndSetting(DataSetBase, metaclass=KeyMeta):
    """### 框架指标元数据配置类（继承 DataSetBase，使用元类 Meta 管理）
        ...
        >>> df = IndFrame(raw_data, lines=["ma5", "ma10"])  # 自动生成 IndSetting 实例
...
class Broker(Base):
    ...
    def __init__(self, kline: KLine, **kwargs):
        ...
        self.islog: bool = self.account.islog
    ...
    def update(self, size: int, long: bool = True) -> None:
        ...
        if self.btindex < self.length:
            ...
            if not current_position:
                ...
                if self.islog:
                    args = (self.strategy_name, self.symbol, datetime,
                            close, size, comm, self.account.balance)
            ...
            # 平仓
                ...
                if self.islog:
                    args = (self.strategy_name, self.symbol, datetime, close, total_close_size,
                ...
                if size > 0:
                    ...
                    if available < margin+comm:
                        ...
                        self.account._total_profit += value
                        if self.islog:
                            args = (self.strategy_name, self.symbol, datetime, close, size,
                                    comm, self.account.balance)
                            self.Logger().operation_msg('开多' if long else '开空', None, *args)
            # 加仓
            else:
                ...
                self.account._total_profit -= comm
                if self.islog:
                    args = (self.strategy_name, self.symbol, datetime, close, size, comm,
...
class DataFrameSet(DataSetBase):
    """数据集
    pandas_object: Optional[pd.DataFrame] kline对应的pandas对象
    kline_object: Optional[pd.DataFrame] kline蜡烛图pandas对象
    source_object: Optional[pd.DataFrame] 在创建kline最初的pandas对象"""
...
class BtNDFrame:
    ...
    def __iter__(self) -> Iterable[IndSeries, Line, float, bool, Any]:
        if len(self.shape) == 1:
    ...
    def take(self, indices, axis: Axis = 0, **kwargs) -> IndFrame | IndSeries | pd.DataFrame | pd.Series | Any:
        """### 沿指定轴按位置索引获取元素
        ...
        Returns:
            IndFrame | IndSeries | pd.DataFrame | pd.Series | Any: 按位置索引选取的数据子集
    ...
    def filter(
        ...
        regex: str | None = None,
        axis: Axis | None = None,
    ) -> IndFrame | IndSeries | pd.DataFrame | pd.Series:
        """### 根据标签名称过滤数据
        ...
        Returns:
            IndFrame | IndSeries | pd.DataFrame | pd.Series: 过滤后的数据子集
    ...
    def pipe(
        ...
        *args,
        **kwargs,
    ) -> IndFrame | IndSeries | pd.DataFrame | pd.Series | Any:
        """### 链式方法调用管道
        ...
        Returns:
            IndFrame | IndSeries | pd.DataFrame | pd.Series | Any: 函数处理后的结果
...
class PandasDataFrame(BtNDFrame, pd.DataFrame):
    ...
    def eval(self, expr: str, *, inplace: bool = False, **kwargs) -> IndFrame | Any:
        """### 执行字符串表达式计算
    ...
    def select_dtypes(self, include=None, exclude=None, **kwargs) -> IndFrame:
        """### 根据数据类型选择列

        基于列的数据类型筛选DataFrame的列。

        Args:
            include: 包含的数据类型列表（如['number', 'object']）
            exclude: 排除的数据类型列表
    ...
    def drop(
            ...
            **kwargs) -> IndFrame | pd.DataFrame | Any:
        """### 删除DataFrame中的指定行或列
    ...
    def pop(self, item: Hashable, **kwargs) -> IndSeries:
        """### 弹出指定列
    ...
    #     Example:
    #         >>> df1 = PandasDataFrame({'A': [1, 2], 'B': [3, 4]})
    #         >>> df2 = PandasDataFrame({'A': [1, 3], 'B': [3, 5]})
    ...
    def combine(
        ...
        **kwargs
        ...
        Example:
            >>> df1 = PandasDataFrame({'A': [1, 2], 'B': [3, 4]})
            >>> df2 = PandasDataFrame({'A': [5, 6], 'B': [7, 8]})
    ...
    def combine_first(self, other: pd.DataFrame, **kwargs) -> IndFrame:
        ...
        Example:
            >>> import numpy as np
            >>> df1 = PandasDataFrame({'A': [1, np.nan], 'B': [np.nan, 4]})
            >>> df2 = PandasDataFrame({'A': [5, 6], 'B': [7, 8]})
    ...
    def update(
        ...
        **kwargs
        ...
        Example:
            >>> df1 = PandasDataFrame({'A': [1, 2], 'B': [3, 4]})
            >>> df2 = PandasDataFrame({'A': [5, 6]}, index=[0, 1])
            >>> df1.update(df2)
            >>> print(df1)
               A  B
            0  5  3
    ...
    def join(
        ...
        **kwargs
        ...
        Example:
            >>> # 创建索引不完全匹配的DataFrame
            >>> df1 = PandasDataFrame({'A': [1, 2, 3]}, index=['x', 'y', 'z'])
            >>> df2 = PandasDataFrame({'B': [4, 5]}, index=['x', 'y'])
    ...
    def merge(
        ...
        **kwargs
        ...
        Example:
            ...
            >>> df1 = PandasDataFrame({'key': ['a', 'b', 'c'], 'A': [1, 2, 3]})
            >>> df2 = PandasDataFrame({'key': ['a', 'b', 'c'], 'B': [4, 5, 6]})
    ...
    # 以下操作axis !=0返回的是内置数据
    def count(self, axis: Axis = 0, numeric_only: bool = False, **kwargs) -> IndSeries | pd.Series:
        """### 计算非空值数量
    ...
    def all(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | bool: 检查结果，当axis=None时返回标量
    ...
    def min(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | Any: 最小值结果，当axis=None时返回标量
    ...
    def max(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | Any: 最大值结果，当axis=None时返回标量
    ...
    def sum(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | Any: 和的结果，当axis=None时返回标量
    ...
    def prod(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | Any: 乘积结果，当axis=None时返回标量
    ...
    def mean(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | Any: 平均值结果，当axis=None时返回标量
    ...
    def median(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | Any: 中位数结果，当axis=None时返回标量
    ...
    def sem(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | Any: 标准误差结果，当axis=None时返回标量
    ...
    def var(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | Any: 方差结果，当axis=None时返回标量
    ...
    def std(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | Any: 标准差结果，当axis=None时返回标量
    ...
    def skew(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series | Any: 偏度结果，当axis=None时返回标量
    ...
    def kurt(
        ...
        **kwargs,
        ...
        Returns:
            IndSeries | pd.Series Any: 峰度结果，当axis=None时返回标量
    ...
    def nunique(self, axis: Axis = 0, dropna: bool = True) -> IndSeries | pd.Series:
        """### 计算唯一值数量
...
class PandasSeries(BtNDFrame, pd.Series):
    ...
    def reset_index(
        ...
        **kwargs
        ...
        Returns:
            IndFrame | IndSeries | Any: drop=False时返回DataFrame，drop=True时返回Series
