...
if TYPE_CHECKING:
    ...
    class PT:
        def __call__(self, *args, **kwargs)->PairTrading:...
...
class CoreFunc:
    ...
    def pair_hurst_filter(self,window=20, hurst_threshold=0.5, z_threshold=2.0, **kwargs) -> pd.DataFrame:
        arg, kwarg = self._get_column_(locals(), *FILED.C)
        return self.PairTrading().hurst_filter_strategy(*arg, window, hurst_threshold, z_threshold)

    def pair_kalman_filter(self, y_series=None, window=60, state_var_init=1e-4, obs_var=1e-2, **kwargs) -> pd.DataFrame:
        arg, kwarg = self._get_column_(locals(), *FILED.C)
        return self.PairTrading().kalman_filter(*arg, y_series, window, state_var_init, obs_var)
