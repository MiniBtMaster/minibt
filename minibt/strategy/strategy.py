...
class Strategy(Base, StrategyBase, metaclass=StrategyMeta):
    ...
    def __call__(self: Strategy, *args, **kwds) -> Strategy:
        ...
        if self._isoptimize:
            ...
            return self._execute_live_trading()            # 2. 实盘模式
    ...
    def _init_basic_components_before_start(self):
        ...
        """
        # 初始化K线数据集合（管理所有关联的KLine实例）
        self._btklinedataset = KLinesSet()
        # 初始化指标数据集合（管理所有关联的IndSeries/IndFrame实例）
    ...
    def _init_strategy_data(self):
        ...
        assert self._btklinedataset, '请先通过adddata()方法添加K线数据,或在策略初始化时使用self.get_kline(LocalDatas.test)'
    ...
    def resample(self, cycle: int, data: KLine = None, rule: str = None, **kwargs) -> KLine:
        """
        对外暴露的K线周期转换接口（低→高周期）
        核心作用：提供标准化接口，将指定K线数据转换为目标高周期，返回KLine实例
    ...
    def replay(self, cycle: int, data: KLine = None, rule: str = None, **kwargs) -> KLine:
        """
        对外暴露的数据回放接口（高→低周期）
        核心作用：将高周期K线数据回放为低周期，模拟实时行情，返回KLine实例（实盘模式不生效）
    ...
    def _update_datas(self) -> tuple:
        ...
        index = self._btindex
        sid = self.sid
        live = self._is_live_trading
        if self._strategy_replay:
            items = self._btklinedataset.get_replay_data(index).items()
        else:
            items = self._btklinedataset.items()
        # 遍历所有K线数据，生成绘图数据
        for i, (_, kline) in enumerate(items):
            # 获取TQ实盘数据
            if live:
                tq_data = kline._dataset.tq_object.iloc[-20-length:]
                _datetime = tq_data.datetime.apply(
                    time_to_datetime).values[-length:]
            else:
                tq_data = kline.iloc[-20-length:]
                _datetime = tq_data.datetime.values[-length:]
            volume5 = tq_data.volume.rolling(5).mean().values[-length:]
            volume10 = tq_data.volume.rolling(10).mean().values[-length:]
            ...
            # 整理K线基础绘图数据（含涨跌标记inc）
            if live:
                lh = kline._dataset.tq_object[['low', 'high']]
            else:
                lh = tq_data[['low', 'high']]
            Low = lh.min(1).values[-length:]
            High = lh.max(1).values[-length:]
            data = dict(
                datetime=_datetime,
                open=open,
                high=high,
            ...
            for isplot, ind_name, lines, rlines, doubles, plotinfo in ind_record:
                ...
                ind = self._btindicatordataset[doubles and ind_name[0]
                                               or ind_name].pandas_object
                if self._strategy_replay:
                    ind = ind.iloc[index-length+1:index+1]
                else:
                    ind = ind.iloc[-length:]

                # ========== 核心修复1：重置指标索引为相对索引（0~length-1） ==========
                ind_reset = ind.reset_index(drop=True)  # 重置索引为0开始的相对索引
                ind_values = ind_reset.values  # 指标值用重置后的索引
                ind_index = ind_reset.index.values  # 相对索引（0~length-1）
                ...
                if signal_info:
                    for k, v in signal_info.items():
                        ...
                        if signalshow:
                            ...
                            if signaloverlap:
                                price = tq_data[signalkey].values
                            else:
                                try:
                                    # 用重置索引的指标
                                    price = ind_reset[signalkey].values
                                except:
                                    price = ind_reset[k].values

                            # ========== 核心修复2：基于相对索引筛选信号 ==========
                            svalues = ind_reset[k].values  # 信号值（重置索引后）
                            signal_mask = svalues > 0  # 信号触发的掩码
                            sindex = ind_index[signal_mask]  # 相对索引（0~length-1）
                            sprice = price[signal_mask]  # 信号价格
                            sdatetime = _datetime[signal_mask]  # 信号时间（与K线严格对齐）
                            slen = len(sindex)

                            # ========== 核心修复3：信号index改为相对索引 ==========
                            signal_data = dict(
                                index=sindex,  # 相对索引（0~length-1），不再是原始绝对索引
                                datetime=sdatetime,
                                price=sprice,
                                size=[signalsize,]*slen,
                                text=[signallabel and signallabel["text"]
                                      or "text",]*slen,
                                issignal=slen,
                                # 新增：记录当前K线总长度，用于live_plot对齐
                                total_length=length
                            )
        ...
        return [sid, source], [sid, _btind_span], self._get_account_info() if live else self._account.account_info[index]
    ...
    def btind_like(self, ds: Union[IndSeries, IndFrame, tuple[int], int], **kwargs) -> Union[IndSeries, IndFrame]:
        ...
        核心作用：根据参考数据或维度，生成结构一致的全NaN指标（IndSeries/IndFrame），供用户后续赋值
    ...
    def __setattr__(self, name, value: KLine | IndSeries | IndFrame | Line):
        ...
        if not self._isinit:
            if self._first_start:
                # 参数优化和实盘时，当运行self.get_data时返回内置数据，不需要重新获取数据
