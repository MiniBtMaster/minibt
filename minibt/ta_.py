        ...
        if _StatsmodelsApi is None:
            with model_lock:  # 确保多线程安全
                try:
                    ...
                    raise RuntimeError(
                        '需要安装 statsmodels 才能使用此功能: pip install statsmodels'
                    ) from e
        ...
        if _TqFunc is None:
            with model_lock:  # 确保多线程安全
                try:
                    ...
                    raise RuntimeError(
                        '需要安装 tqsdk 才能使用此功能: pip install tqsdk'
                    ) from e
        return _TqFunc
        ...
        if _sklearn_preprocessing is None:
            with model_lock:
                try:
                    ...
                    raise RuntimeError(
                        '需要安装 scikit-learn 才能使用此功能: pip install scikit-learn'
                    ) from e
        ...
        if _arch_model is None:
            with model_lock:
                try:
                    ...
                    raise RuntimeError(
                        '需要安装 arch 才能使用此功能: pip install arch'
                    ) from e
        return _arch_model
        ...
        if _KalmanFilter is None:
            with model_lock:
                try:
                    ...
                    raise RuntimeError(
                        '需要安装 pykalman 才能使用此功能: pip install pykalman'
                    ) from e
