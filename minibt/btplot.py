...
def btplot(indicator: Union[KLine, Line, IndSeries, IndFrame],
    ...
    is_kline = "KLine" in type(indicator).__name__
    df = None
    if is_kline:
        df = indicator._dataset.source_object
    else:
    ...
    if indicators:
        for i in range(100):
            if "KLine" in type(indicators[-1]._dataset.source_object).__name__:
                ...
                indicators.append(indicators[-1]._dataset.source_object)

        if include == "last":
            indicators = indicators[:1]
    assert df is not None, "找不到K线图"
