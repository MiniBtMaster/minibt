try:
    from bokeh.core.properties import DateTime as Datetime  # 导入 Bokeh 的时间类型
except:
    from bokeh.core.properties import Datetime  # 旧版本：首字母小写 t！
try:
    # Bokeh 2.x/3.x 主流导入方式
...
def make_document(doc: Document):
    global black_style, plot_width, period_milliseconds, click_policy, updatelength
                        ...
                        for k, v in signal_info.items():
                            ...
                            if signalshow:
                                ...
                                signal_ind_datatime_source[i][j].update(
                                    {f"{name}{i}_{k}": np.datetime64(datetime.now()), "index": signal_index[-1]})
   ...
   # 在创建其他组件后添加行情表格
    ...
    def periodic_update():
        ...
        try:
                ...
                for i, update_data in update_datas:
                    if update_data:
                        for j, data in enumerate(update_data):
                            ...
                            last_datetime = update_datetime[-1]
                            # 定义默认的信号切片长度（从data中获取）
                            default_signal_length = len(data.get('close', 10))
                            # 初始化patch和stream所需数据结构

                            if live:
                                # ========== 核心修改：按新K线判断逻辑重构live模式 ==========
                                stream_data = {}  # 用于增量数据追加
                                source_last_dt = source_datetime[-1]
                                # 1. 判定是否有新K线（更新数据最后一条时间 > 源数据最后一条时间）
                                has_new_kline = False
                                # 统一时间格式对比（消除精度差异）
                                source_last_dt = pd.Timestamp(
                                    source_last_dt)  # .isoformat()
                                update_last_dt = pd.Timestamp(
                                    last_datetime)  # .isoformat()
                                has_new_kline = update_last_dt > source_last_dt

                                if has_new_kline:
                                    bar = int(
                                        (update_last_dt-source_last_dt).seconds/cycles[i][j])
                                    for k, v in source[i][j].data.items():
                                        if k != 'index':
                                            value = data[k]
                                            # 取更新数据的最后一条作为新K线值
                                    ...
                                    stream_data['index'] = list(
                                        range(current_max_length, current_max_length+bar))
                                    # print("*"*100)
                                    # print("前",
                                    ...
                                    _source_data = source[i][j].data
                                    source_datetime = source[i][j].data['datetime']
                                    ...
                                    # print("后", source[i][j].data["index"][-updatelength:], range(
                                    #     target_patch_length-len(value), target_patch_length))

                            else:
                                has_new_kline = True
                                stream_data = {}
                                # 非live模式：默认追加最新值，走stream
                                for k, v in _source_data.items():
                                    if k != 'index':
                                        if isinstance(data[k], (list, np.ndarray)):
                                            # 取最新的一个值
                                            value = data[k][-1] if len(
                                                data[k]) > 0 else v[-1]
                                        else:
                                            value = data[k]
                                ...
                                # 执行stream追加
                                if stream_data:
                                    current_max_idx = _source_data['index'][-1] if len(
                                        _source_data['index']) > 0 else -1
                                    stream_data['index'] = [
                                        current_max_idx + 1]
                                    source[i][j].stream(
                                        stream_data, rollover=None)
                            soruce_datas = source[i][j].data
                            update_length = len(
                                soruce_datas.get('close', []))

                            for signalkey, signalsource in signal_ind_data_source[i][j].items():
                                if signalkey in data:
                                    current_datetime = signal_ind_datatime_source[i][j].get(
                                        signalkey)
                                    current_index = signal_ind_datatime_source[i][j].get(
                                        "index")
                                    signalsource_data = signalsource.data
                                    signalinfo = data[signalkey]
                                    issignal = signalinfo.pop("issignal")
                                    signaldata = {}
                                    slen = 0
                                    if issignal:
                                        signal_datetime = signalinfo["datetime"]
                                        last_signal_datetime = signal_datetime[-1]
                                        if last_datetime == last_signal_datetime:
                                            if live:
                                                # 获取策略端传递的相对索引和总长度
                                                signal_rel_index = signalinfo["index"]
                                                signal_total_length = signalinfo.get(
                                                    "total_length", default_signal_length)
                                                # 相对索引转绝对索引 + 边界检查
                                                sindex = signal_rel_index + update_length - signal_total_length
                                                sindex = np.unique(
                                                    sindex)  # 去重
                                                # 过滤负数
                                                ...
                                                # 对齐最后索引
                                                sindex = signalinfo["index"]
                                                sindex = sindex + \
                                                    update_length - \
                                                    sindex[-1] - 1
                                            signaldata["index"] = np.union1d(
                                                signalsource_data["index"], sindex)
                                            slen = len(
                                                signaldata["index"])-len(signalsource_data["index"])
                                            for k, v in signalsource_data.items():
                                                if k != 'index':
                                                    value = signalinfo[k][-slen:]
                                                    signaldata[k] = np.append(
                                                        v, value)
                                            signal_ind_datatime_source[i][j]["index"] = signaldata["index"][-1]

                                        elif current_datetime == last_datetime and current_index in signalsource_data["index"][-10:]:
                                            for k, v in signalsource_data.items():
                                                signaldata[k] = v[:-1]
                                            slen = 1
                                        ...
                                        for k, v in signalsource_data.items():
                                            signaldata[k] = v[:-1]
                                        slen = 1
                                    if signaldata and slen:
                                        signalsource.data = signaldata
                                        signal_ind_datatime_source[i][j][signalkey] = last_datetime

                            if has_new_kline and ispoint:
                                # 检查当前策略/合约的更新状态（默认允许）
                                # update_allowed = doc.plot_update_state.get(
                                #     (i, j), True)
                                # if update_allowed:
            ...
            if account_info:
                aid = loadData(id_dir)
                div.update(text=account_info[aid])
    ...
    #                         # last_datetime = update_datetime[-1]
    #                         # # 获取当前K线总长度
    #                         # current_kline_length = len(
    #                         #     _source_data.get('close', []))
    #                         # # 定义默认的信号切片长度（从data中获取）
    #                         # default_signal_length = len(data.get('close', 10))

    #                         # if live:
    #                         #     _source_datetime = source_datetime[-updatelength-1:]
    #                         #     isintime = update_datetime[0] in _source_datetime
    #                         #     if isintime:
    #                         #         source_index = len(source_datetime)-updatelength-1+list(_source_datetime).index(
    #                         #             update_datetime[0])
    #                         #     for k, v in _source_data.items():
    #                         #         if k != 'index':
    #                         #             value = data[k]
    #                         #             soruce_datas[k] = np.append(
    #                         #                 v[:source_index] if isintime else v, value)
    #                         # else:
    #                         #     for k, v in _source_data.items():
    #                         #         if k != 'index':
    #                         #             if isinstance(data[k], (list, np.ndarray)):
    ...
    #                         current_high = data["high"][-1]
    #                         current_low = data["low"][-1]
    #                         # soruce_datas = {}  # 保留原有变量名，后续代码需使用
    #                         update_datetime = data['datetime']
    #                         source_datetime: np.ndarray = _source_data['datetime']
    #                         last_datetime = update_datetime[-1]
    #                         # 获取当前K线总长度
    #                         # current_kline_length = len(
    #                         #     _source_data.get('close', []))
    ...
    #                         patch_data = {}  # 用于存量数据修改
    #                         stream_data = {}  # 用于增量数据追加
    #                         source_last_dt = source_datetime[-1] if len(
    ...
    #                             else:
    #                                 # 源数据为空时，默认是新K线
    #                                 has_new_kline = len(update_datetime) > 0

    #                             # 2. 预处理：构建时间→索引映射（仅用于精准定位最后1-2根K线）
    #                             source_datetime_str = [pd.Timestamp(
    #                                 ts).isoformat() for ts in source_datetime]
    #                             source_dt_to_idx = {
    #                                 ts_str: idx for idx, ts_str in enumerate(source_datetime_str)}

    #                             if has_new_kline:
    #                                 # ---------- 有新K线：先追加新K线，再patch倒数第二根 ----------
    #                                 # 步骤1：构建stream数据（仅追加最新的一条新K线）
    #                                 for k, v in _source_data.items():
    #                                     if k != 'index':
    #                                         value = data[k]
    #                                         # 取更新数据的最后一条作为新K线值
    #                                         stream_value = value[-1] if isinstance(
    #                                             value, (list, np.ndarray)) else value
    #                                         stream_data[k] = [stream_value]
    ...
    #                                     source_datetime = _source_data['datetime']
    #                                     source_dt_to_idx = {
    #                                         pd.Timestamp(ts).isoformat(): idx
    #                                         for idx, ts in enumerate(source_datetime)
    #                                     }

    #                                 # 步骤2：构建patch数据（更新倒数第二根K线）
    #                                 # 倒数第二根K线的索引（追加新K线后，原最后一根变为倒数第二根）
    #                                 target_patch_idx = len(
    #                                     source_datetime) - 2 if len(source_datetime) >= 2 else 0
    #                                 if target_patch_idx >= 0:
    #                                     for k, v in _source_data.items():
    #                                         if k != 'index':
    #                                             value = data[k]
    #                                             # 取更新数据的倒数第二条作为倒数第二根K线的新值（无则取最后一条）
    #                                             patch_value = value[-2] if (isinstance(
    #                                                 value, (list, np.ndarray)) and len(value) >= 2) else value[-1]
    #                                             patch_data[k] = [
    #                                                 (target_patch_idx, patch_value)]
    ...
    #                                 # 定位最后一根K线的索引
    #                                 target_patch_idx = len(
    #                                     source_datetime) - 1 if len(source_datetime) > 0 else 0
    #                                 if target_patch_idx >= 0:
    #                                     for k, v in _source_data.items():
    #                                         if k != 'index':
    #                                             value = data[k]
    #                                             # 取更新数据最后一条替换最后一根K线
    #                                             patch_value = value[-1] if isinstance(
    #                                                 value, (list, np.ndarray)) else value
    #                                             patch_data[k] = [
    #                                                 (target_patch_idx, patch_value)]
    ...
    #                             # 同步更新soruce_datas
    #                             # updated_data = source[i][j].data
    #                             # for k in updated_data.keys():
    #                             #     soruce_datas[k] = updated_data[k]
    #                         soruce_datas = source[i][j].data
    #                         # 保持原有update_length计算逻辑
    #                         update_length = len(soruce_datas.get('close', []))
