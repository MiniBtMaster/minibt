...
import weakref
from .indicators import pd, datetime, Optional, partial, FILED, Union, Iterable
...
class SqliteDict(DictClass):
    ...
    def __init__(self, conn: SqliteMultithread, tablename='unnamed', flag='c'):
        """
        ...
        self.conn = conn
        if self.flag == 'r':
            if self.tablename not in self.conn.tablenames:
        ...
        if flag == 'w':
            self.clear()
        self._last_item_string = f"SELECT COUNT(*) FROM {self.tablename}"
    ...
    def __str__(self):
        return "SqliteDict(%s)" % (self.tablename)
    ...
    def iteritems(self):
        ...
        item = self.conn.select(GET_ITEMS)
        if item:
            for key, value in self.conn.select(GET_ITEMS):
                yield identity(key), decode(value)
    ...
    @property
    def _row(self):
        self.conn.cursor.execute(f"SELECT count(*) FROM {self.tablename}")
        fetchone = self.conn.cursor.fetchone()
        return fetchone[0]

    def _get_length_string(self, length) -> str:
        end = self._row
        start = max(1, end-length)
        return f"SELECT value FROM {self.tablename} LIMIT {start},{length}"

    def _get_date_string(self, start: str, end: str = None) -> str:
        assert start
        if end:
            return f"select value from {self.tablename} where between '{start}' and '{end}'"
        else:
            return f"select value from {self.tablename} where key>='{start}'"

    def get_data(self, start: str = None, end: str = None, length: int = 0) -> pd.DataFrame:
        if length > 0:
            string = self._get_length_string(length)
        else:
            string = self._get_date_string(start, end)
        item = self.conn.select(string)
        df = pd.DataFrame([decode(value[0]) for value in item])
        df.datetime = pd.to_datetime(df.datetime)
        return df

    def last_value(self) -> dict:
        item = self.conn.select_one(
            f"SELECT value FROM {self.tablename} where rowid=={self._row}")
        return decode(item[0])

    def last_key(self) -> str:
        item = self.conn.select_one(
            f"SELECT key FROM {self.tablename} where rowid=={self._row}")
        return identity(item[0])

    def last_data(self) -> tuple[str, dict]:
        item = self.conn.select_one(
            f"SELECT key,value FROM {self.tablename} where rowid=={self._row}")
        return identity(item[0]), decode(item[1])

    def __contains__(self, key):
        HAS_ITEM = 'SELECT 1 FROM "%s" WHERE key = ?' % self.tablename
        return self.conn.select_one(HAS_ITEM, (identity(key),)) is not None

    def __getitem__(self, key):
        GET_ITEM = 'SELECT value FROM "%s" WHERE key = ?' % self.tablename
        item = self.conn.select_one(GET_ITEM, (identity(key),))
        if item is None:
            raise KeyError(key)
        return decode(item[0])
    ...
    def __setitem__(self, key, value):
        ...
        self.conn.execute(ADD_ITEM, (identity(key), encode(value)))
        # if self.autocommit:
        #     self.commit()
    ...
    def __delitem__(self, key):
        ...
        self.conn.execute(DEL_ITEM, (identity(key),))
        # if self.autocommit:
        #     self.commit()

    def update(self, name="", items=(), **kwds):
        if self.flag == 'r':
        ...
        try:
            ...
            pass
        items = [(identity(k), encode(v)) for k, v in items]
        # UPDATE_ITEMS = 'REPLACE INTO "%s" (key, value) VALUES (?, ?)' % self.tablename
        self.conn.cursor.executemany(
            f"insert into {name} (key ,value) values (?,?);", items)
        # for k, v in items:
        #     self.conn.execute(UPDATE_ITEMS, (identity(k), encode(v)))
    ...
    def clear(self):
        ...
        # self.conn.conn.commit()
        self.drop_table()
        self.__init__(self.conn, self.tablename, self.flag)
...
class SqliteMultithread(threading.Thread):
    ...
    def tablenames(self):
        ...
        #     res = cursor.fetchall()
        cursor = self.conn.execute(GET_TABLENAMES)
        res = cursor.fetchall()
    ...
    def _connect(self):
        ...
        try:
            ...
            self.log.exception(
                "Failed to initialize connection for filename: %s" % self.filename)
...
class MiniDB:
    ...
    def __init__(self, filename: str = "./minibt/data/data.sqlite", flag='c',
                 autocommit=True, journal_mode="DELETE", outer_stack=True) -> None:
        self.in_temp = filename is None
        self.filename = filename
        if self.in_temp:
            fd, self.filename = tempfile.mkstemp(prefix='sqldict')
            os.close(fd)
        ...
        self.tablename: str = 'unnamed'
        self.flag = flag
        self.conn = SqliteMultithread(
            self.filename, autocommit, journal_mode, outer_stack)
        self.db = SqliteDict(self.conn, self.tablename, self.flag)
    ...
    def get_table(self, tablename='unnamed'):
    ...
    def get(self, name, start: Optional[str] = None, end: Optional[str] = None, length: Optional[int] = 0) -> pd.DataFrame:
        return self.get_table(name).get_data(start, end, length)

    def update(self, name='unnamed', items: pd.DataFrame = None):
        if isinstance(items, pd.DataFrame) and not items.empty:
            items.datetime = pd.to_datetime(
                items.datetime).apply(self.timetostr_func)
            items = items[FILED.Quote]
            d = items.to_dict(orient="records")
            data = list(zip(items.datetime, d))
            self.get_table(name).update(name, data)
        elif isinstance(items, Iterable) and items and len(items[0]) == 2:
            self.get_table(name).update(name, items)
