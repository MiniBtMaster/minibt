...
class LightChartWindow(QWidget):
    ...
    def __init__(self, parent: MainWindow = None, strategyes: list[Strategy] = None, period_milliseconds: int = 1000):
        ...
        self.drawings = {}
        self.visible_range = {}
        self.mouse_label_color = None
        self.bear_color = btcolors.bear_color
        self.bull_color = btcolors.bull_color
    ...
    def __initWidget(self):
        ...
        self.chart_window = Chart(
            self, strategy=self.strategyes[0], period_milliseconds=self.period_milliseconds)
    ...
    def _do_chart_replacement(self, strategy: Strategy):
        self.chart_window = Chart(
            self, strategy=strategy, period_milliseconds=self.period_milliseconds)
...
class Chart(QtChart):
    ...
    def kline_update(self) -> None:
        """K线更新函数"""
        latest_tick_time = self.datetime.iloc[-1]
        chang = self.is_changing(
            self, latest_tick_time)
        kline = self.kline[FILED.ALL]
        # 更新频率高的条件放前面
        if chang == 0:
    ...
    def _switch_to_latest_300_kline(self,):
        ...
        from_ts = pd.to_datetime(
            datetime.iloc[-300]).timestamp()
        to_ts = pd.to_datetime(datetime.iloc[-1]).timestamp()
        self.run_script(f'''
